...
Most source datasets report per-level GPS coordinates, so that the horizontal position of the sonde is known at each altitude level as it drifts with the wind during its flight.  A small fraction of older files (notably 306 of 13{,}678 files in the 1996--2012 hurricane archive) lack position data entirely, and IGRA radiosondes never carry per-level coordinates.  Rather than retain the native coordinates for some datasets and not others, we compute the horizontal drift track uniformly for every profile by integrating the gridded horizontal winds in time, anchored at the launch position, and use the datasets that do report per-level coordinates as an end-to-end check (\S\ref{sec:validation}).

For a single profile, let $\{\bar{z}_k\}$ denote the cell-center altitudes and $\{t_k\}$ the bin-averaged observation times.  The altitude bins are reindexed in ascending order of $t_k$; under this ordering, dropsondes progress downward and radiosondes upward without the need to distinguish the two.  The drift at the earliest-time bin is set equal to the launch position, and the drift at successive bins is accumulated using the trapezoidal rule on the gridded winds:
\begin{align}
...
When $u$ or $v$ is missing at a bin, the integration carries forward the last valid value across the gap.  A running budget records the horizontal distance accumulated during the current gap, $d_\text{gap} = \sqrt{(\Delta x)^2 + (\Delta y)^2}$ summed over consecutive missing-wind steps, and is reset to zero as soon as a valid wind value is encountered.  Once $d_\text{gap}$ exceeds $\SI{1}{km}$, the integration is abandoned and all remaining bins (in time order) are marked as missing.  This threshold limits the horizontal error that can be incurred by extrapolating through a single unobserved region.
...
A monthly climatological profile of zonal and meridional wind is attached to every sounding as a reference against which the instantaneous measurements can be assessed.  The climatology is derived from ERA5 monthly means over 1995--2025 on a $0.25^\circ$ lat/lon grid and a \SI{100}{m} altitude grid from 0 to \SI{40}{km}, constructed in the sibling repository \texttt{era5-climatology} by re-projecting monthly ERA5 reanalysis onto altitude per column.  Values below the ERA5 surface orography are stored as missing.

For each launch location $(\phi_0, \lambda_0)$ and calendar month $m$, the climatological wind is obtained by (i) bilinear interpolation of the ERA5 slab for month $m$ onto $(\phi_0, \lambda_0)$, then (ii) linear interpolation of the resulting column in altitude onto the output cell centers $\bar{z}_k$.  Target altitudes above the ERA5 ceiling or below the local ERA5 surface orography remain missing.

Attaching the climatology is implemented as a post-processing step, separate from the regridding pipeline.  Once the regridded NetCDF files have been produced by \texttt{src/process.py}, the companion script \texttt{src/attach\_climatology.py} iterates over them, interpolates the climatology one ERA5 variable at a time (so that only one $\sim 20$ GB slab resides in memory at a time), and rewrites each NetCDF with the new fields appended.  This split means the climatology can be regenerated (e.g.\ against a newer ERA5 product) without re-running the regridding, and vice versa.

The storage layout differs between dataset types.  For dropsonde campaigns, each sounding has its own launch location and time, and the climatology for the relevant month is stored per sounding; the variables \texttt{u\_clim} and \texttt{v\_clim} share the \texttt{(sounding\_id, altitude)} shape of the directly gridded fields.  For IGRA station files, the station location is fixed but individual soundings span many different months; storing a separate profile per sounding would duplicate the same column many times at each station, so instead all twelve monthly profiles are stored once at the station location with shape \texttt{(month, altitude)}, where \texttt{month} is a new coordinate indexing calendar months $1\dots 12$.  Profiles for which a launch location or launch time is not available are stored as missing, which includes the synthetic SAM and multifractal datasets.
