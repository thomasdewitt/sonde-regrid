...
The drift integration described in \S\ref{sec:drift} is checked against the per-level GPS coordinates reported in the source files, for each dataset whose raw data contains them.  For a given dataset, a random sample of 200 native (altitude, latitude, longitude) triples is drawn from 20 randomly chosen profiles, restricted to levels at which all three fields are finite.  Each native altitude is matched to the nearest bin center in the regridded output, and the great-circle distance between the integrated and measured positions is computed using the haversine formula.  The test asserts that the maximum distance is below $\SI{500}{m}$ for every dataset, and also records the RMS distance across all sampled points.
