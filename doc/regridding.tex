...
The regridded data are stored as one NetCDF-4 file per source dataset.  Each file has three dimensions: \texttt{launch\_location}, \texttt{launch\_time}, and \texttt{altitude}.
For radiosonde datasets, every launch from a given station shares the \texttt{launch\_location} coordinate. The size of the \texttt{launch\_time} dimension is 1 for dropsonde datasets given that each dropsonde is launched from a different spatial location.
